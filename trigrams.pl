#!/usr/bin/perl
#
# (c) Tomasz Miklas <miklas.tomasz __AT__ gmail.com>
# Version: 1.0
...
#
sub load_trigrams {
	# populate %trigs from DATA section below
	while (<DATA>) {
		next if /^;/;
		chomp;
		my ($t,$s) = m/^(.*?)\s+(\d+)$/;
...
sub trigrams($) {
	...
	my $result = 0;
	map {
		# count occurences of trigram, multiply by trigram score and add to the result
		$result += $trigs{$_} * scalar $text =~ /$_/i;
	} keys %trigs;
	return $result;
}
...
# below section is a mirror of http://home.ccil.org/~cowan/trigrams as of Mon Sep  8 09:30:30 UTC 2014
__DATA__
